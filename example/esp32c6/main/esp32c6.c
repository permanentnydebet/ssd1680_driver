...
#include "FontLiberationSerif24.h"
...
bool SpiInit(void) {
    ...
    if (spi_bus_add_device(LCD_HOST, &devcfg, &spi) != ESP_OK)
        return false;
...
void app_main(void) {
  ...
  EPD_ShowString(image, 15, 15, "Nothing!", FontLiberationSerif24, kEpdColorBlack);
  EPD_ShowString(image, 15, 50, "Hello world!", FontLiberationSerif24, kEpdColorRed);
